"""Mayhem serial interface helper class.

Provides MayhemSerial — a standalone class that owns the pyserial connection to
Mayhem firmware over a CDC-ACM serial port, a daemon reader thread, and all
command methods (applist, appstart, setfreq, radioinfo).

Independently usable without ROS2 or Redis.
"""
...
class MayhemSerial:
    ...
    Usage::
        ...
        ms = MayhemSerial('/dev/ttyACM1', timeout=3.0)
        if ms.open():
    ...
    def __init__(self, port: str, timeout: float = 3.0) -> None:
        ...
        """
        self._port = port
        self._logger = logging.getLogger('pymayhem.serial')
    ...
    def open(self) -> bool:
        ...
        try:
            ...
            # brief pause so reader thread is ready before first command
            time.sleep(0.05)
            return True
    ...
    def needs_reconnect(self) -> bool:
        ...
        Poll this to detect when the serial port has disconnected
