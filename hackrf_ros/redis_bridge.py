"""Redis I/O bridge helper class for HackRF ROS2 driver.

Provides RedisBridge — a standalone class that owns the redis-py connection,
a daemon thread for all Redis I/O, and command dispatch to HackRFNode.
HackRFNode uses this class so it never calls redis-py directly.
...
class RedisBridge:
    ...
    def __init__(self, iq_queue: queue.Queue, node, logger,
        ...
        Args:
            ...
            node: HackRFNode reference for command dispatch.
    ...
    def needs_reconnect(self) -> bool:
        ...
        HackRFNode polls this to detect when Redis has disconnected
        and a re-open is needed.
        """
        return self._stop_event.is_set() and self._thread is not None

    # ------------------------------------------------------------------
    # State publishing (thread-safe — called from ROS2 executor)
    # ------------------------------------------------------------------

    def publish_state(self, state: dict) -> None:
        """HSET hackrf:state with the given mapping.

        Thread-safe: may be called from the ROS2 executor or any other thread.
    ...
    def _dispatch_command(self, cmd: dict) -> None:
        """Route a parsed command dict to the appropriate HackRFNode method.
